...
import { 
  ...
  Trash2, Clock, Star, ArrowLeft, MoreVertical, File
} from "lucide-react";
...
type Analysis = {
  sentiment: "Positive" | "Negative" | "Neutral"; category: string; confidenceScore: number; recommendedAction: string; extractedDetails: string[]; draftedReply: string;
};

const getCategoryColor = (analysis: Analysis | null | undefined) => {
  if (!analysis) return 'bg-slate-50 text-slate-600 border-slate-200/80';
  const category = analysis.category.toUpperCase();
  if (category.includes('COMPLAINT') && !category.includes('FOLLOW')) return 'bg-rose-50 text-rose-600 border-rose-200/60';
  if (category.includes('FOLLOW')) return 'bg-emerald-50 text-emerald-600 border-emerald-200/60';
  if (category.includes('FEEDBACK') || analysis.sentiment === 'Positive') return 'bg-blue-50 text-blue-600 border-blue-200/60';
  if (analysis.sentiment === 'Negative') return 'bg-rose-50 text-rose-600 border-rose-200/60';
...
export default function SupportDesk() {
  ...
  return (
    <div className="h-screen w-screen flex flex-col overflow-hidden bg-[#F4F5F7] font-sans selection:bg-zinc-200 selection:text-zinc-900 text-zinc-900 relative p-4 lg:p-6">
      ...
      <div className="flex-1 w-full max-w-[1600px] mx-auto bg-white rounded-[2rem] shadow-[0_8px_40px_rgba(0,0,0,0.08)] border border-zinc-200 flex flex-col overflow-hidden relative z-10">
        ...
        <header className="h-16 px-6 flex items-center justify-between shrink-0 bg-white relative z-20 border-b border-zinc-100">
          ...
          <div className="flex items-center gap-4 w-64 justify-end">
            ...
            <div className="w-9 h-9 rounded-full bg-gradient-to-br from-indigo-50 to-fuchsia-50 border border-white shadow-sm flex items-center justify-center font-bold text-xs text-indigo-700 ml-2 relative overflow-hidden">
                  ...
                  filteredEmails.map(email => (
                    <div key={email.id} onClick={() => handleSelectEmail(email)} className="flex items-center gap-4 px-6 py-4 bg-white/70 hover:bg-white border border-white shadow-[0_2px_10px_rgba(0,0,0,0.02)] hover:shadow-md cursor-pointer group text-[14px] relative transition-all rounded-2xl">
                      ...
                      <div className="flex-1 min-w-0 flex items-center pr-4">
                        <span className="font-extrabold text-zinc-900 mr-3 truncate shrink-0 max-w-[280px] tracking-tight text-[15px]">{email.subject}</span>
                        {analysisMap[email.id] && (
                           <span className={`px-2.5 py-1 rounded-md text-[10px] font-bold uppercase mr-3 shrink-0 tracking-widest shadow-sm ${getCategoryColor(analysisMap[email.id])}`}>
                              {analysisMap[email.id]?.category}
                           </span>
                        )}
                        <span className="text-zinc-600 truncate flex-1 font-medium">- {email.content}</span>
                      </div>
                    ...
                    <div className="bg-white rounded-[24px] shadow-[0_8px_30px_rgba(0,0,0,0.06)] border border-white overflow-hidden flex flex-col">
                      <div className="flex items-center justify-between p-6 bg-zinc-50/50 border-b border-zinc-100/50">
                        ...
                        <div className="text-right">
                          ...
                          <div className="text-[12px] text-zinc-400 mt-0.5 font-bold uppercase tracking-widest">to me</div>
                        </div>
                      </div>
      ...
      {/* Toast Notification */}
      {toast && (
        <div className={`fixed top-6 right-6 px-5 py-3.5 rounded-2xl shadow-xl flex items-center gap-3 z-50 animate-in slide-in-from-top-8 ${
